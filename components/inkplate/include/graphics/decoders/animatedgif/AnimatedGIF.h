...
// If you intend to decode generic GIFs, you want this value to be 12. If you
// are using GIFs solely for animations in your own project, and you control the
// GIFs you intend to play, then you can save additional RAM here: the decoder
// must reserve a minimum of 4 byte * (1<<MAX_CODE_SIZE) for the dictionary, but
// based on implementation actually reserves 5 byte * (1<<MAX_CODE_SIZE). Small
// or low colour GIFs may inherently not require a large dictionary. For larger
// GIFs, the en(!)coder can "voluntarily" choose not to utilize the entire
// dictionary. I.e., by preparing (specially encoding) the GIFs, you can save
// >10kB RAM, but you will not be able to decode arbitrary images anymore. One
// application to craft such GIFs can be found here (use option -d)
...
// Draw types
//
// RAW = 8-bit palettized pixels requiring transparent pixel handling and
// conversion through the palette.
//       Each line is sent to the GIFDraw callback as 8-bit pixels. If a
//       framebuffer exists, the lines will be written there too. The GIFDraw
//       callback is optional if there is a framebuffer allocated.
//
// COOKED = 16/24/32-bpp fully rendered pixels ready for display. This requires
// a full frame buffer with extra
//          room for the fully rendered pixels at the end of the 8-bit pixel
//          buffer. For example, a 160x120 canvas size with 24-bit output would
//          require (160*120 + 3*160) bytes. Each prepared line is sent to the
//          GIFDraw callback as a row of 16/24/32-bit pixels.
//
...
typedef struct gif_draw_tag {
  ...
  int iCanvasWidth; // need this to know where to place output in a fully cooked
                    // bitmap
  ...
  uint8_t ucIsGlobalPalette; // Flag to indicate that a global palette, rather
                             // than a local palette is being used
} GIFDRAW;
...
typedef struct gif_image_tag {
  ...
  uint16_t iPrevW, iPrevH, iPrevX,
      iPrevY;      // for disposal method 2 (restore to bg)
  ...
  unsigned char ucGIFBits, ucBackground, ucTransparent, ucCodeStart, ucMap,
      bUseLocalPalette;
  ...
  unsigned short pPalette[(MAX_COLORS * 3) /
                          2]; // can hold RGB565 or RGB888 - set in begin()
  unsigned short
      pLocalPalette[(MAX_COLORS * 3) / 2]; // color palettes for GIF images
...
public:
...
#endif
  int open(const char *szFilename, GIF_OPEN_CALLBACK *pfnOpen,
           GIF_CLOSE_CALLBACK *pfnClose, GIF_READ_CALLBACK *pfnRead,
           GIF_SEEK_CALLBACK *pfnSeek, GIF_DRAW_CALLBACK *pfnDraw);
  void close();
  void reset();
  ...
  void mergeTransparent(uint8_t *pSrc, uint8_t *pDst, uint8_t ucTrans,
                        int iLen);
  void cookPixels(uint8_t *pSrc, uint8_t *pDst, int iTrans, int iLen,
                  uint32_t *pPalette, uint16_t *pRGB565);
...
#else
// C interface
int GIF_openRAM(GIFIMAGE *pGIF, uint8_t *pData, int iDataSize,
                GIF_DRAW_CALLBACK *pfnDraw);
int GIF_openFile(GIFIMAGE *pGIF, const char *szFilename,
                 GIF_DRAW_CALLBACK *pfnDraw);
...
void GIF_mergeTransparent(uint8_t *pSrc, uint8_t *pDst, uint8_t ucTrans,
                          int iLen);
void GIF_cookPixels(uint8_t *pSrc, uint8_t *pDst, int iTrans, int iLen,
                    uint32_t *pPalette, uint16_t *pRGB565);
#endif // __cplusplus
