 ...
 *              periodic deep-sleep updates on Soldered Inkplate 6.
 *
 * @details     Ported from the Inkplate-Arduino-library's
 *              Inkplate6_OpenMeteo_Weather_Station example. Connects
 *              Inkplate 6 to WiFi, synchronizes local time via NTP (using
 ...
 *   the display either, and Gui.cpp's layout coordinates (e.g. fillRect
 *   calls reaching x=800) confirm it targets the native 800x600 landscape
 *   canvas, not a rotated 600x800 portrait one -- rotating would clip the
 *   majority of the dashboard's width instead. One quirk carried over
 *   unmodified from the original Arduino sketch: the day/night indicator
 *   box in Gui::displayWeatherData() is drawn at y=645-795, which exceeds
 *   the panel's native height of 600 px and is therefore silently clipped
 *   (invisible) on real hardware. This is a pre-existing layout bug in the
 *   upstream example, not something introduced by this port, so Gui.cpp was
 *   left unchanged to match it verbatim.
 *
