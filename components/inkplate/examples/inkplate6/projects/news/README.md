...
Fetch top news headlines from NewsAPI.org over WiFi and render a "World News" layout on Soldered Inkplate 6, refreshing periodically via deep sleep.

## Overview

Connects Inkplate 6 to WiFi (credentials configured via menuconfig), synchronizes time via NTP, and fetches the `top-headlines` endpoint from [NewsAPI.org](https://newsapi.org/) through the `NetworkFunctions` helper (`main/Network.cpp` + `main/Network.h`), which performs the HTTPS request with `esp_http_client` (verified against the ESP-IDF certificate bundle) and parses the JSON response with cJSON.
