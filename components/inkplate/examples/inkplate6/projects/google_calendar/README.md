...
- Orientation: `display.setRotation(1)` is called in `app_main()` to match the original sketch's portrait layout — the drawing coordinates in `Gui.cpp` assume a 600x800 portrait screen.
...
- Event titles longer than `MAX_SUMMARY_LENGTH` (in `Gui.h`, default 16 characters, matching the original sketch) are truncated with "...".
