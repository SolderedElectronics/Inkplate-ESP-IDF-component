 ...
 *              Calendar API.
 *
 * Ported from the Inkplate6_Google_Calendar Arduino example. The original
 * used HTTPClient + ArduinoJson; this port performs the same single HTTP GET
 * with esp_http_client and parses the JSON response with cJSON. Note that
 * the original example (and this port) authenticates with a simple Google
 * API key against a *public* calendar - it does not use the OAuth2
 * refresh-token flow needed for private calendars.
 */
#pragma once
