...
TRMNL BYOS ("Bring Your Own Server") client for Soldered Inkplate 6COLOR.

## Overview

Connects Inkplate 6COLOR to WiFi, registers with a self-hosted Terminus (TRMNL BYOS) server via `/api/setup`, then fetches and displays whatever screen the server assigns via `/api/display`, deep-sleeping between refreshes on the interval the server specifies.
...
- This panel has no fast partial-refresh call, so WiFi-connect progress goes to the log only, not the screen.
