 ...
 *              a simple HTTP server. Visiting the server IP in a browser shows a
 *              form where text can be submitted. The submitted text is displayed
 *              on the e-paper screen in real time. The AP SSID, password, and
 *              server IP are shown on the screen at startup.
 *
 * Requirements:
...
static esp_err_t handle_string(httpd_req_t *req)
{
    ...
    if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK)
    {
        ...
        if (httpd_query_key_value(query, "text", param, sizeof(param)) == ESP_OK)
        {
            ...
            int wi = 0;
            for (int ri = 0; param[ri] && wi < (int)sizeof(g_userText) - 1; ri++, wi++)
            {
                if (param[ri] == '+')
                    g_userText[wi] = ' ';
