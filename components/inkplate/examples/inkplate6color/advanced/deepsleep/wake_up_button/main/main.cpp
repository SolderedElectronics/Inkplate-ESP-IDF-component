 ...
 * @brief       Deep sleep with button or timer wakeup for Soldered Inkplate 6Color.
 *
 * @details     Demonstrates two wakeup sources for ESP32 deep sleep: a 30-second
 *              timer and the hardware wake-up button (GPIO 36). On each wakeup
 *              the boot count (stored in RTC memory) is incremented and the wakeup
 *              cause is printed on the display. After updating the screen the board
 ...
 * 2) The board shows the boot count and wakeup reason, then sleeps for 30 seconds.
 * 3) Press the wake-up button to wake the board early.
 *
 * Expected output:
 * - Boot count and wakeup reason ("timer" or "WakeUp button") displayed on screen.
 *
 * Notes:
