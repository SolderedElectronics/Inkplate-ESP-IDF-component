...
bool NetworkFunctions::spotifyGetCurrentlyPlaying(const char *accessToken,
  ...
  if (album) {
    ...
    // Spotify's "images" array is sorted largest-first (typically 640,
    // 300, and 64 px). Index 1 (~300px) is a good size/quality tradeoff for
    // the ~300x300 cover slot drawn in Gui.cpp, matching the original
    // sketch's choice. cJSON_GetArrayItem() safely returns NULL if the
    ...
    if (cJSON_IsArray(images) && cJSON_GetArraySize(images) > 0) {
      cJSON *img1 = cJSON_GetArrayItem(images, 1);
      cJSON *urlObj = img1 ? cJSON_GetObjectItem(img1, "url") : nullptr;
