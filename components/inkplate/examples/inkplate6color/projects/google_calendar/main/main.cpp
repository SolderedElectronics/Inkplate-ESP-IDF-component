 ...
 *
 * @details     Connects Inkplate 6Color to WiFi, syncs the wall clock via
 *              SNTP, then requests upcoming events from a *public* Google
 *              Calendar using the Google Calendar API (Calendar ID + API
 *              key) and renders a day/agenda view on the 7-color e-paper
 *              screen.
 *
 ...
 *
 * Notes:
 * - Display mode: Inkplate 6Color's native 7-color e-paper mode (there is no
 *   setDisplayMode() call on this board - it always renders in color).
 ...
 *   a 448x600 portrait screen (the native panel is 600x448 landscape).
...
// Color used to highlight the currently ongoing event. Any INKPLATE_* color
// works: INKPLATE_BLACK, INKPLATE_WHITE, INKPLATE_GREEN, INKPLATE_BLUE,
// INKPLATE_RED, INKPLATE_YELLOW, INKPLATE_ORANGE (see Inkplate6Color.h).
#define HIGHLIGHT_COLOR INKPLATE_ORANGE
