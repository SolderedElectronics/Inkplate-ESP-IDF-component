...
```cpp
#define HIGHLIGHT_COLOR INKPLATE_ORANGE // INKPLATE_BLACK/WHITE/GREEN/BLUE/RED/YELLOW/ORANGE
```
...
- Display: a black header bar showing the current day-of-month (large), day of week, month/year, and "Last Updated" time, followed by upcoming events for the next 14 days grouped by day (date + short day name), each with a title and start/end time. An event currently in progress is drawn with a rounded highlight box in `HIGHLIGHT_COLOR`. If there are no more events in the next two weeks, a message says so.
...
- Display mode: Inkplate 6Color is a 7-color e-paper board (black, white, green, blue, red, yellow, orange via the `INKPLATE_*` color macros). There is no `setDisplayMode()` call on this board — it always renders in its native color mode.
- Orientation: `display.setRotation(1)` is called in `app_main()` to match the original sketch's portrait layout — the drawing coordinates in `Gui.cpp` assume a 448x600 portrait screen (the native panel is 600x448 landscape). This was verified by checking the original sketch's `Gui.cpp` coordinates: several `y` values (up to ~575) exceed the native 448 px height but fit comfortably within the 600 px portrait height, while all `x` values fit within the 448 px portrait width — confirming the layout was authored for portrait orientation, and matching the original sketch's own explicit `setRotation(1)` call.
...
- Event titles longer than `MAX_SUMMARY_LENGTH` (in `Gui.h`, default 20 characters, matching the original sketch) are truncated with "...".
