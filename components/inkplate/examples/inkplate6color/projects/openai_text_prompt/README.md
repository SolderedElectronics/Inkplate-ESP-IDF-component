...
Send a text prompt to the OpenAI Chat Completions API over HTTPS and display the response on Soldered Inkplate 6Color.

## Overview

Connects Inkplate 6Color to WiFi, sends a fixed text prompt to OpenAI's `/v1/chat/completions` endpoint, and renders the returned text on the e-paper screen using a custom font (`FreeMonoBold18pt7b`).
...
- Inkplate 6Color is a 7-color e-paper board (600x448), so this port uses `INKPLATE_BLACK` / `INKPLATE_WHITE` instead of the generic `BLACK`/`WHITE` macros, and there is no `setDisplayMode()` call since this board does not have selectable display modes like the monochrome boards.
