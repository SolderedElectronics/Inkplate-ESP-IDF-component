 ...
 * @brief       Generates and draws a random maze in black lines on Soldered
 *              Inkplate 6Color.
 *
 * @details     Creates a new random maze on every boot and renders it to the
 *              Inkplate 6Color e-paper display. A simple maze generation
 ...
 *
 * Notes:
 * - The maze is drawn using only black lines on a white background; this
 *   example does not use the panel's accent colors (green, blue, red,
 *   yellow, orange).
 * - This example performs a single full refresh and then stays idle.
 *   Inkplate 6Color does not support partial updates.
...
static const int cellSize = 19;

// Margin (in pixels) left empty on every side of the maze.
static const int margin = 15;

// Maze grid dimensions, derived from the panel resolution and the cell size.
// Inkplate 6Color's 600x448 panel yields a 30x22 cell grid, filling the
// display edge-to-edge with an even 15px margin on all sides:
//   mazeWidth  = (600 - 2*15) / 19 = 570 / 19 = 30
//   mazeHeight = (448 - 2*15) / 19 = 418 / 19 = 22
    ...
    for (int x = 0; x < width; ++x) {
      ...
      for (int i = 0; i < 4; ++i) {
        ...
        if (xx >= 0 && xx < width && yy >= 0 && yy < height &&
          ...
          display.drawLine(
              3 + x * cellSize + cellSize / 2 + margin,
              3 + y * cellSize + cellSize / 2 + margin,
              3 + x * cellSize + cellSize / 2 + (kDirX[i] * cellSize / 2) +
                  margin,
              3 + y * cellSize + cellSize / 2 + (kDirY[i] * cellSize / 2) +
                  margin,
              INKPLATE_BLACK);
        }
      }
