...
Fetch top news headlines from NewsAPI.org over WiFi and render a "World News" layout on Soldered Inkplate 6Color, refreshing periodically via deep sleep.

## Overview

Connects Inkplate 6Color to WiFi (credentials configured via menuconfig), synchronizes time via NTP, and fetches the `top-headlines` endpoint from [NewsAPI.org](https://newsapi.org/) through the `NetworkFunctions` helper (`main/Network.cpp` + `main/Network.h`), which performs the HTTPS request with `esp_http_client` (verified against the ESP-IDF certificate bundle) and parses the JSON response with cJSON.

`main.cpp` then renders a simple newspaper-style screen: a "World News" title, the current date and last-update time, and a list of headline/description boxes drawn with `drawTextBox()` using two custom fonts (`FreeSerifItalic24pt7b` for the title, `FreeSerifBold12pt7b` for the headlines, `Inter10pt7b` for the date row and descriptions). The display is rotated with `setRotation(3)` before drawing, matching the original Arduino sketch; all layout coordinates are computed from `display.width()`/`display.height()` after that rotation, so they adapt to the resulting canvas automatically.
...
- Inkplate 6Color is a 7-color e-paper board (black, white, green, blue, red, yellow, orange) with a single, fixed color display mode — there is no `setDisplayMode()` call on this board. This example only draws with `INKPLATE_BLACK`/`INKPLATE_WHITE`, matching the original sketch's layout, and does not support partial updates (every refresh is a full refresh via `display.display()`).
- Deep sleep resets the ESP32 on every wake — there is no `loop()`; all logic runs once per boot inside `app_main()`.
- `display.wifi.setCurrentTime()` sets the system TZ internally, but `time()` always returns UTC seconds regardless of TZ, so this example applies `TIMEZONE_OFFSET_HOURS` manually (same approach as this component's Inkplate 6 `news` example, and equivalent to the original sketch's fixed `timeZone` variable).
