...
Generate an image from a text prompt using OpenAI's image generation API (DALL-E), download it, and display it on Soldered Inkplate 6Color. Repeats periodically via RTC-scheduled deep sleep.

## Overview

Connects Inkplate 6Color to WiFi, sends a JSON request to OpenAI's `/v1/images/generations` endpoint with a text prompt, parses the JSON response to extract the generated image's URL, then downloads and renders that image on the 7-color e-paper display.
...
Status messages ("Connecting...", "Generating image...", etc.) and the final downloaded image are all shown with full refreshes (`display.display()`) — Inkplate 6Color does not support partial updates at all. The board then schedules its next wake-up using the on-board RTC alarm and enters deep sleep, so the whole flow repeats from `app_main()` on every wake-up.
...
- During startup: short status messages on the display, each pushed with a full refresh (this board has no partial update mode).
- After generation: the downloaded image rendered on the e-paper display, dithered down to the panel's 7-color palette (black, white, green, blue, red, yellow, orange).
...
- Color image path: Inkplate 6Color uses `ImageColor` instead of the plain grayscale `Image` class, so `display.image.draw()` dithers/quantizes the downloaded image down to the panel's 7 colors rather than grayscale shades. The `draw()` signature is the same shape as on grayscale boards: `draw(url, x, y, dither, invert)`.
- Display mode: there is no `setDisplayMode()` / black-and-white / grayscale concept on this board, and `partialUpdate()` is a no-op, so every status update and the final image use a full `display.display()` refresh, matching the original Arduino sketch's behavior for this board.
...
- The requested image size is 1024x1024, but it's centered using a 512 px half-width/half-height offset, matching the original sketch. On Inkplate 6Color's 600x448 panel this offset is negative on both axes, so the image ends up heavily cropped on all sides rather than merely clipped top/bottom as on larger boards; this quirk is carried over from the original rather than "fixed" here.
...
- Wake-up is configured via RTC alarm epoch and an external wake on GPIO 39 (tied to the RTC interrupt line on Inkplate 6Color).
