 ...
 * @brief       OpenAI image generation slideshow for Soldered Inkplate 6Color.
 *
 * @details     Connects Inkplate 6Color to WiFi, sends a text prompt to
 *              OpenAI's image generation API (DALL-E), parses the JSON
 *              response to extract the generated image's URL, downloads that
 *              image, and renders it on the 7-color e-paper display. The
 ...
 *              refreshes (display.display()) — Inkplate 6Color does not
 *              support partial updates at all (its partialUpdate() is a
 *              no-op), unlike the black & white / grayscale boards.
 *
 ...
 *   each time — this board has no partial update mode).
 * - After generation: the downloaded image rendered on the e-paper display,
 *   dithered down to the panel's 7-color palette.
 * - Serial output includes the OpenAI HTTP status/response and the resolved
 *   image URL.
 *
 * Notes:
 * - Color image path: Inkplate 6Color uses ImageColor (not the plain
 *   grayscale Image class), so display.image.draw() dithers/quantizes the
 *   downloaded image down to the panel's 7 colors (black, white, green,
 *   blue, red, yellow, orange) instead of grayscale shades. The draw()
 *   signature is the same shape as on grayscale boards:
 *   draw(url, x, y, dither, invert).
 * - Display mode: there is no setDisplayMode()/BLACK_AND_WHITE/GRAYSCALE
 *   concept on this board, and partialUpdate() is a no-op, so every status
 *   update and the final image are pushed with a full display.display()
 *   refresh, matching the original Arduino sketch's behavior for this board.
 ...
 *   Inkplate 6Color's 600x448 panel this offset is negative on both axes
 *   (display.width()/2 - 512 and display.height()/2 - 512), so the image is
 *   heavily cropped on all sides rather than merely clipped top/bottom as on
 *   larger boards. This quirk is carried over from the original rather than
 *   "fixed" here.
...
extern "C" void app_main(void) {
  ...
  if (gotUrl) {
    ...
    // Image assumed to be 1024x1024; offset to center it (matches original).
