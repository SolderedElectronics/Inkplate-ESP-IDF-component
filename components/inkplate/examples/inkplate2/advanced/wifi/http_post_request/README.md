...
Send periodic HTTP POST requests from Inkplate 2 to webhook.site.

## Overview

Connects Inkplate 2 to a WiFi network and sends periodic HTTP POST requests to [webhook.site](https://webhook.site) — a free online service for inspecting HTTP requests in real time. Useful for testing IoT data transmission without a dedicated backend.
...
- Serial Monitor logs WiFi connection status and POST results.
