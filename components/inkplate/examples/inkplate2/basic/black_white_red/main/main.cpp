...
extern "C" void app_main(void)
{
    ...
    displayCurrentAction(&display, "Drawing two pixels in different colors"); // Function which writes small text at bottom left
    ...
    for (int i = 0; i < 30; i++)
    {
        display.drawLine(rnd(0, 211), rnd(0, 80), rnd(0, 211), rnd(0, 80), rnd(1, 3));
    }
    ...
    for (int i = 0; i < 20; i++)
    {
        display.drawThickLine(rnd(0, 211), rnd(0, 80), rnd(0, 211), rnd(0, 80), INKPLATE2_BLACK,
                              (float)rnd(1, 20));
    }
    ...
    displayCurrentAction(&display, "Drawing a grid using horizontal and vertical lines");
    display.display();
    vTaskDelay(pdMS_TO_TICKS(DELAY_MS));
