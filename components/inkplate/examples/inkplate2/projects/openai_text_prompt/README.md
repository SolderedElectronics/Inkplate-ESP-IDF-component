...
Send a text prompt to the OpenAI Chat Completions API over HTTPS and display the response on Soldered Inkplate 2.

## Overview

Connects Inkplate 2 to WiFi, sends a fixed text prompt to OpenAI's `/v1/chat/completions` endpoint, and renders the returned text on the e-paper screen using a custom font (`FreeMonoBold18pt7b`).
...
- The `FreeMonoBold18pt7b` font is large relative to Inkplate 2's small 104x212 panel, so only a short response will fit on screen. Keep `PROMPT_TEXT` (and the expected reply) short.
