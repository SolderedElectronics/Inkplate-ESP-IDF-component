...
Fetch a random quote from a public API and display it on Soldered Inkplate 2, refreshing periodically via deep sleep.

## Overview

Connects Inkplate 2 to WiFi, performs an HTTP GET against the [Quotable](https://github.com/lukePeavey/quotable) public API (no API key required), and parses the JSON response with cJSON to extract the quote text and author. The quote is rendered inside a word-wrapped text box using a custom script font, and the author name is printed in the bottom-right corner.
...
- Display: the fetched quote centered in a text box using a custom script font (LoveLetter_Regular10); the author printed in the bottom-right corner, prefixed with "-".
...
- The API is public and requires no key; its response format may change over time — update `NetworkFunctions.cpp` if parsing starts failing.
