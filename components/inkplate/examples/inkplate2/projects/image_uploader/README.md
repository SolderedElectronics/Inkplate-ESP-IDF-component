...
Turn Soldered Inkplate 2 into a standalone web app for uploading an image from a phone or PC and displaying it on the e-paper screen.

## Overview

Inkplate 2 connects to a WiFi network and starts an HTTP server (using ESP-IDF's native `esp_http_server` component). The server serves an upload page that lets the user take a photo or choose an image from their gallery. The page scales and crops the image to the panel resolution in the browser, then uploads it to the device. The device decodes the uploaded JPEG and renders it on the e-paper display.
...
Open `http://<printed-ip>/` in a web browser on a device connected to the same network. The upload page lets you take a photo or choose an image from your gallery; pressing **Upload** sends it to the device, which decodes it and renders it on the Inkplate 2 e-paper display.
...
- The upload page (`main/html.h`) scales/crops the picture to the Inkplate 2 panel resolution (212x104) client-side before uploading, so JPEGs stay small.
