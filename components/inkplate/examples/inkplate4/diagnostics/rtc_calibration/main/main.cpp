 ...
 *
 * @details     Provides a step-by-step procedure for calibrating the RTC crystal
 *              oscillator. After pressing the wake-up button, the RTC is zeroed
 *              and the elapsed time is displayed every second, alternating between
 *              partial and full updates. The internal capacitor and clock offset
 *              registers can be adjusted to correct drift. Detailed calculation
 *              instructions are provided in the source code comments.
 *
 * Requirements:
 ...
 *
 * Expected output:
 * - “Press wake button to start RTC!” on screen; HH:MM:SS displayed after button
 *   press, updating every second.
 *
 * Notes:
...
void printTime(Inkplate &display, uint8_t _hour, uint8_t _minutes, uint8_t _seconds)
{
    // Print time
...
void app_main(void)
{
    ...
    display.setTextSize(3); // Set text to be 5 times bigger than classic 5x7 px text
    gpio_set_direction(GPIO_NUM_36, GPIO_MODE_INPUT); // Set wake-up button as input
