 ...
 * @brief       Comprehensive grayscale graphics demo for Soldered Inkplate 4TEMPERA.
 *
 * @details     Cycles through a wide variety of Adafruit GFX drawing primitives
 *              in 3-bit grayscale mode (8 shades, 0 = black, 7 = white). Includes
 *              pixels, lines, thick lines, gradient lines, grids, rectangles,
 *              circles, rounded rectangles, triangles, ellipses, polygons, bitmap
 *              images, and text in different sizes and shades. Each shape is shown
 ...
 * - Extra:      image_ex.h header with a grayscale image array included in the project
 *
 * Configuration:
 ...
 * 2) The display cycles through all drawing primitives in grayscale automatically.
 *
 * Expected output:
 * - A series of screens demonstrating pixels, lines, shapes, and text in 8-shade grayscale.
 *
 * Notes:
...
void app_main(void)
{
  ...
  while(true)
  {
    ...
    displayCurrentAction(display, "Drawing a pixel"); // Function which writes small text at bottom left indicating what's currently done
    ...
    displayCurrentAction(display, "Drawing 50 lines, random colors and thickness");
    display.display();
    vTaskDelay(pdMS_TO_TICKS(DELAY_MS));
    ...
    displayCurrentAction(display, "Drawing rounded rectangles with random colors");
    display.display();
    vTaskDelay(pdMS_TO_TICKS(DELAY_MS));
    ...
    display.clearDisplay();
    display.image.draw(picture1, 50, 139, 500, 332, BLACK); // Arguments are: array variable name, start X, start Y,  size X, size Y
