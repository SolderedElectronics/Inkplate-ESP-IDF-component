 ...
 *              string into "message.txt" stored on a FAT-formatted SD card.
 *
 * Requirements:
 ...
 * 4) A file named "message.txt" is created (or overwritten if it exists).
 * 5) The text string defined in the code is written into the file.
 *
 * Expected output:
 * - File "message.txt" created on the SD card with written content.
 *
 * Notes:
...
extern "C" void app_main(void) {
  ...
  fprintf(f, "Hello from Inkplate 4TEMPERA!\n");
  fclose(f);
