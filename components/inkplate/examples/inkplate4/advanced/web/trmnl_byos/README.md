...
TRMNL BYOS ("Bring Your Own Server") client for Soldered Inkplate 4TEMPERA.

## Overview

Connects Inkplate 4TEMPERA to WiFi, registers with a self-hosted Terminus (TRMNL BYOS) server via `/api/setup`, then fetches and displays whatever screen the server assigns via `/api/display`, deep-sleeping between refreshes on the interval the server specifies.
