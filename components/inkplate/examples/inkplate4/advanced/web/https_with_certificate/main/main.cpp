 ...
 *              validation. The example connects Inkplate 4TEMPERA to WiFi,
 *              applies a PEM certificate (certificate pinning / trust anchor),
 *              then downloads and renders a BMP image from a website that
 *              matches the provided certificate. It also shows a failed download
 *              case when attempting to load an image from a different host where
 ...
 *    image.
 * 4) A second download attempt demonstrates failure when the certificate does
 *    not match.
 *
 * Expected output:
...
extern "C" void app_main(void) {
  ...
  // Set certificate so HTTPS requests validate against this trust anchor
