 ...
 *
 * @details     Connects the Inkplate 4TEMPERA to WiFi (credentials configured
 *              via menuconfig), synchronizes time via NTP, and fetches
 *              top headlines from NewsAPI.org through NetworkFunctions
 ...
 * 4) Build and flash to Inkplate 4TEMPERA.
...
static void drawNews(Inkplate &display, news *entities, int count) {
  display.setRotation(0); // Portrait mode, matches the original sketch.
  ...
  for (int i = 0; i < maxBoxes && i < count; i++) {
    ...
    display.drawTextBox(leftMargin, y0 + 65, display.width() - rightMargin - 50,
