/**
 * @file        Gui.cpp
 * @brief       Draws the Open-Meteo weather dashboard on Inkplate 4TEMPERA.
 *
 ...
 *              by the original sketch's INKPLATE_3BIT mode.
 */
...
void Gui::drawTemperaturePrecipGraph(WeatherData *weatherData, NetworkFunctions::UserInfo *userInfo)
{
    ...
    // Find actual min/max for temperature
    for (int i = 0; i < 6; i++)
    {
...
void Gui::displayWeatherData(WeatherData *weatherData, NetworkFunctions::UserInfo *userInfo)
{
    ...
    // Loop through the 5-day forecast and display each day
    for (int i = 0; i < 5; i++)
    {
        inkplate.setFont(&FreeSans12pt7b);
