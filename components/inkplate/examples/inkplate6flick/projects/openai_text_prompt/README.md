...
Send a text prompt to the OpenAI Chat Completions API over HTTPS and display the response on Soldered Inkplate 6 Flick.

## Overview

Connects Inkplate 6 Flick to WiFi, sends a fixed text prompt to OpenAI's `/v1/chat/completions` endpoint, and renders the returned text on the e-paper screen using a custom font (`FreeMonoBold18pt7b`).
...
- Inkplate 6 Flick's 800x600 panel gives the `FreeMonoBold18pt7b` font plenty of room, so the reply wraps and renders comfortably starting near the top-left corner.
