...
In `main.cpp`, `IMAGE_URL` defaults to a public test endpoint (`loremflickr.com`) that redirects to a random 1024x758 photo on every request, matching the Inkplate 6 Flick panel resolution. Change it to point to your own image if desired.
...
- `display.image.draw()` downloads the file with `esp_http_client`, which follows HTTP redirects automatically, so no manual redirect-resolution code is needed even though the default URL redirects to a random image on another host. (The original Arduino sketch resolved the `Location` redirect header by hand before downloading; that step is dropped here as unnecessary.)
