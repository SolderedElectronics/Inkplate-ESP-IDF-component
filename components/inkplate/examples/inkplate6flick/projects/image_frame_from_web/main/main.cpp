 ...
 * @brief       Digital picture frame for Soldered Inkplate 6 Flick.
 *
 * @details     Connects Inkplate 6 Flick to WiFi, downloads a JPEG image
 *              from a URL, and renders it full-screen in 3-bit grayscale.
 *              After displaying the image, the ESP32 sets a wake-up timer
 *              and enters deep sleep. When the timer expires, the ESP32
 *              restarts from app_main(), causing the image to be downloaded
 *              and displayed again — creating a periodically refreshing
 *              image frame.
 *
 * Requirements:
 * - Board:      Soldered Inkplate 6 Flick
 * - Framework:  ESP-IDF v6.x
 * - Hardware:   Inkplate 6 Flick, USB cable (or battery for low-power testing)
 ...
 *   a random image on another host. (The original Arduino sketch resolved
 *   the "Location" redirect header by hand before downloading; that step
 *   is unnecessary here.)
...
// Public test endpoint that redirects to a random 1024x758 photo, matching
// Inkplate 6 Flick's panel resolution. Replace with a URL pointing to your
// own image if desired.
#define IMAGE_URL "http://loremflickr.com/1024/758"
