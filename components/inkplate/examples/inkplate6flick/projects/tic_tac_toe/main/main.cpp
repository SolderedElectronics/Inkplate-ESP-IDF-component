 ...
 *              partial updates on Soldered Inkplate 6 Flick.
 *
 ...
 * - The touchscreen (Cypress CY8CTMA140 controller) is initialized and
 *   powered on automatically as part of constructing `Inkplate display;` —
 *   no explicit `touchscreen.init()` call is needed on this port.
...
int firstHuman = -1;  // 0 = human plays first, 1 = computer plays first.
...
void menuEvents(Inkplate &display) {
  ...
  if (display.touchscreen.touchInArea(553, 246, 300, 55) &&
  ...
  if (display.touchscreen.touchInArea(553, 326, 300, 55) &&
  ...
  if (display.touchscreen.touchInArea(585, 627, 301, 79)) { // Start game
...
void gameEvents(Inkplate &display) {
  ...
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 3; ++j)
      if (display.touchscreen.touchInArea(268 + 243 * j, 20 + 243 * i, 243,
                                           243)) {
