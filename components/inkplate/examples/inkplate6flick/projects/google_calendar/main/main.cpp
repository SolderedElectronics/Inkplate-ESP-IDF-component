 ...
 *
 * @details     Connects Inkplate 6 Flick to WiFi, syncs the wall clock via
 *              SNTP, then requests upcoming events from a *public* Google
 *              Calendar using the Google Calendar API (Calendar ID + API
 *              key) and renders a day/agenda view on the e-paper screen.
 *
 ...
 * - Orientation: the original sketch's setRotation(1) call is commented out
 *   by default, but its Gui.cpp layout coordinates (e.g. y up to ~975) only
 *   fit within the 1024 px tall side of the panel, not the native 758 px
 *   landscape height - so this port calls setRotation(1) for portrait mode,
 *   same as the original author intended (see the comment left in the
 *   .ino: "Portrait mode | if it's upside down do setRotation(3)").
...
extern "C" void app_main(void) {
  ...
  display.setRotation(1);            // Portrait mode, matches the layout in Gui.cpp.
