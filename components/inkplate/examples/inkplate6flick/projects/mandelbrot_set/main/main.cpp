 ...
 * @brief       Render a Mandelbrot set view on Soldered Inkplate 6 Flick in
 ...
 *   the first refresh.
 * - The Inkplate 6 Flick panel is 1024x758 (~1.35:1). The original Arduino
 *   sketch's viewing window was close to square, which would stretch
 *   noticeably on a non-square panel if mapped directly. xFrom/xTo below
 *   have been widened around the same window center used by the original
 *   sketch so that (xTo - xFrom) / (yTo - yFrom) matches
 *   E_INK_WIDTH / E_INK_HEIGHT, keeping the fractal undistorted.
...
// Inkplate 6 Flick panel's 1024x758 (~1.35:1) resolution instead of the
// original near-square window, which would otherwise appear vertically
// stretched.
static const double xFrom = -0.726078, xTo = -0.862522;
