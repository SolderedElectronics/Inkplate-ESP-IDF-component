 ...
 * @brief       Secure HTTPS image download using a pinned certificate (Inkplate 6 Flick).
 *
 ...
 *              validation. The example connects Inkplate 6 Flick to WiFi,
 *              applies a PEM certificate (certificate pinning / trust anchor),
 *              then downloads and renders a BMP image from a website that
 *              matches the provided certificate. It also shows a failed
 *              download case when attempting to load an image from a different
 *              host where the certificate is not valid.
 *
 * Requirements:
 ...
 * 4) A second download attempt demonstrates failure when the certificate
 *    does not match.
 *
 * Expected output:
 ...
 *   unrelated domains.
 * - If the certificate is outdated/rotated by the server, you must update it.
 *
