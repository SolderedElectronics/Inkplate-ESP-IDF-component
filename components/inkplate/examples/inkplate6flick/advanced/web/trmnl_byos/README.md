...
TRMNL BYOS ("Bring Your Own Server") client for Soldered Inkplate 6 Flick.

## Overview

Connects Inkplate 6 Flick to WiFi, registers with a self-hosted Terminus (TRMNL BYOS) server via `/api/setup`, then fetches and displays whatever screen the server assigns via `/api/display`, deep-sleeping between refreshes on the interval the server specifies.
