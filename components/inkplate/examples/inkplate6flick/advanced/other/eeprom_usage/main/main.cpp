 ...
 * @brief       NVS (EEPROM equivalent) read/write example for Soldered Inkplate 6 Flick.
 *
 * @details     Demonstrates how to use ESP32 Non-Volatile Storage (NVS) on
 *              Inkplate 6 Flick to store data that persists across resets and
 *              power cycles. This is the ESP-IDF equivalent of the Arduino
 *              EEPROM library. The example shows how to clear, write, and read
 *              user data from NVS using key-value pairs.
 *
 * Requirements:
...
extern "C" void app_main(void) {
  ...
  if (err == ESP_ERR_NVS_NO_FREE_PAGES ||
    ...
    ESP_LOGW(TAG, "NVS partition problem (%s), erasing...",
             esp_err_to_name(err));
    nvs_flash_erase();
    nvs_flash_init();
  }
