...
Send a text prompt to the OpenAI Chat Completions API over HTTPS and display the response on Soldered Inkplate 10.

## Overview

Connects Inkplate 10 to WiFi, sends a fixed text prompt to OpenAI's `/v1/chat/completions` endpoint, and renders the returned text on the e-paper screen using a custom font (`FreeMonoBold18pt7b`).
...
- Inkplate 10's 1200x825 panel gives the `FreeMonoBold18pt7b` font even more room than on Inkplate 6, so the reply wraps and renders comfortably starting near the top-left corner.
