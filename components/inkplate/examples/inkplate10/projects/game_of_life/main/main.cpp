 ...
 * - MIN_CELLSZ/MAX_CELLSZ are scaled up from the Inkplate 6 version's 8-40 px
 *   range to 12-60 px, roughly matching the 1200/800 = 1.5x increase in
 *   horizontal resolution. This keeps the on-screen cell size (and therefore
 *   the total cell count and per-generation computation) comparable to the
 *   Inkplate 6 port instead of quadrupling the workload on the larger panel.
 ...
 *   On the Inkplate 10's 1200x825 panel these grids total roughly
 *   2 * (1200/12) * (825/12) ~= 13.6 KB, comfortably fitting in RAM.
 *
...
// Scaled up from the Inkplate 6 port's 8/40 px range by roughly the
// 1200/800 = 1.5x increase in horizontal resolution, so cells occupy a
// similar proportion of the (larger) screen and the total cell count stays
// comparable rather than growing with the full increase in pixel area.
#define MIN_CELLSZ 12
#define MAX_CELLSZ 60
...
// Inkplate 10's 1200x825 panel these grids are modestly sized and
