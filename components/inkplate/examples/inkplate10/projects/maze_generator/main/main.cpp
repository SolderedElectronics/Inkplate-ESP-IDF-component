 ...
 *              Inkplate 10 e-paper display. A simple maze generation
...
// increase generation and drawing time. Kept the same as the Inkplate 6
// example (20px) because both panels have a similar pixel density (~150-167
// DPI), so this preserves a comparable physical cell size on paper/screen
// while naturally yielding more cells on Inkplate 10's larger panel.
...
// Inkplate 10's 1200x825 panel yields a raw 57x38 cell grid; 38 is even, so
// mazeHeight is rounded down to 37 to close the bottom border.
static const int rawMazeWidth = (E_INK_WIDTH - 60) / cellSize;
static const int rawMazeHeight = (E_INK_HEIGHT - 60) / cellSize;
