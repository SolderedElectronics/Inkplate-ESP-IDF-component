...
bool NetworkFunctions::spotifyGetCurrentlyPlaying(const char *accessToken,
  ...
  if (album) {
    ...
    // 300, and 64 px). Index 0 (~640px) matches the 640x640 cover slot
    // drawn in Gui.cpp, matching the original sketch's choice of the
    // largest available image. cJSON_GetArrayItem() safely returns NULL if
    // the array has fewer entries than expected.
