...
- **Layout scaling:** Inkplate 10's panel (1200x825) is 1.5x wider and 1.375x taller than Inkplate 6's (800x600), with a different aspect ratio. The original Soldered Arduino sketch for Inkplate 10 reuses Inkplate 6's fixed pixel margins verbatim, which leaves disproportionate empty space around the quote box on the bigger panel. This port instead scales the quote box's side margins and its vertical reach above/below center by those same width/height ratios (see `BOX_SIDE_MARGIN_PX`, `BOX_TOP_OFFSET_PX`, `BOX_BOTTOM_OFFSET_PX`, and the author signature margins in `main.cpp`). The font-metric-derived constants (`BOX_LINE_SPACING_PX`, `BOX_WRAP_FONT_SIZE`) are left unchanged, since they describe the fixed-size bitmap fonts themselves, not the panel.
