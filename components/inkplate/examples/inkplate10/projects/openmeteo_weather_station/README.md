...
- **Orientation (no rotation):** `app_main()` deliberately does **not** call `display.setRotation(...)`. The original Arduino sketch never rotates the display either, and `Gui.cpp`'s layout coordinates confirm it: several `fillRect`/`drawBitmap` calls reach `x = 1175` / `y = 795`, which only fits the native 1200x825 **landscape** canvas — rotating to a portrait orientation would instead clip most of the dashboard's width.
- **Layout already sized for the 1200x825 panel:** unlike this component's Inkplate 6 `openmeteo_weather_station` port — whose upstream Arduino sketch reuses the same fixed panel coordinates and therefore clips its day/night indicator box off the bottom of the smaller 800x600 canvas — the upstream Inkplate 10 sketch's `Gui.cpp` already targets this panel's full resolution: the main-info/user-info panels span `x = 25`–`1175`, the additional-info/graph/weekly-forecast row spans down to `y = 795`, and the day/night box at `y = 645`–`795` fits comfortably within the panel's 825 px height. So `Gui.cpp` is ported with its coordinates unchanged rather than rescaled.
