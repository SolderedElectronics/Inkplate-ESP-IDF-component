/**
 * @file        Gui.cpp
 * @brief       Draws the Open-Meteo weather dashboard on Inkplate 10.
 *
 * @details     Ported from the Inkplate10_OpenMeteo_Weather_Station Arduino
 *              example's src/Gui.cpp. Drawing calls are unchanged
 *              (Adafruit_GFX-compatible); WeatherData/UserInfo fields are
 *              now fixed-size char buffers instead of Arduino `String`, but
 *              print()/println() accept `const char*` directly so the
 *              call sites did not need to change. Color values 0 (black)
 *              through 7 (white) are the same 3-bit grayscale levels used
 *              by the original sketch's INKPLATE_3BIT mode. Layout values
 *              are unchanged from the original sketch: unlike the
 *              Inkplate 6 sibling port (whose upstream sketch reused the
 *              same fixed panel coordinates as this one and therefore
 *              clips its day/night indicator box off the bottom of the
 *              smaller 800x600 canvas), the upstream Inkplate 10 sketch
 *              already targets the panel's native 1200x825 canvas: the
 *              "user info" panel extends to x=1175, the "additional
 *              info"/graph/weekly-forecast row extends to y=795, and the
 *              day/night box at y=645-795 fits comfortably above the
 *              bottom margin. So this port keeps every coordinate as-is
 *              instead of recomputing a new layout.
 */
...
{
    // main info (top-left) with an outline
    inkplate.fillRect(25, 25, 800, 200, 5);
    inkplate.drawRect(25, 25, 800, 200, 0);

    // user info / battery (top-right)
    inkplate.fillRect(775, 25, 400, 200, 0);

    // additional info (left column)
    inkplate.fillRect(25, 225, 350, 420, 0);

    // hourly temperature/precipitation graph (right of additional info) with an outline
    inkplate.fillRect(375, 225, 800, 420, 5);
    inkplate.drawRect(375, 225, 800, 420, 0);

    // weekly forecast (bottom row, right of the day/night indicator)
...
{
    // Layout values for graph placement (fills the right-hand panel drawn by
    // drawBackground(): x=375-1175, y=225-645).
    int graphX = 425;
    int graphY = 280;
    int graphWidth = 700;
    int graphHeight = 360;
...
void Gui::displayWeatherData(WeatherData *weatherData, NetworkFunctions::UserInfo *userInfo)
{
    ...
    {
        inkplate.setFont(&FreeSans12pt7b);
        int xPos2 = startX + i * dayWidth;

        // Day name
        inkplate.setCursor(xPos2 + 15, startY);
        inkplate.setTextColor(7);
        inkplate.print(weatherData->dailyNames[i]);

        // Weather icon
        inkplate.setFont(&FreeSans9pt7b);
        inkplate.drawBitmap(xPos2 + 15, startY + 20, getWeatherIcon(weatherData->dailyWeatherCodes[i]), iconSize,
                            iconSize, 7);
