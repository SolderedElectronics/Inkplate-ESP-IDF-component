 ...
 *
 * @details     Connects the Inkplate 10 to WiFi (credentials configured via
 *              menuconfig), synchronizes time via NTP, and fetches top
 ...
 * 4) Build and flash to Inkplate 10.
 ...
 *   adapt automatically to the resulting canvas size. On Inkplate 10
 *   (E_INK_WIDTH=1200, E_INK_HEIGHT=825) this rotation swaps the axes, so
 *   display.width() is 825 and display.height() is 1200 - the same values
 *   the original Inkplate10_News.ino sketch's layout constants were tuned
 *   for, so they are carried over as-is rather than rescaled from the
 *   Inkplate 6 port (whose canvas, at 600x800 after rotation, is smaller).
...
static void drawNews(Inkplate &display, news *entities, int count) {
  ...
  int textWidth = strlen("World News") * 12;
  int centerX = (display.width() - textWidth) / 2;
  display.setCursor(centerX - 70, 60);
  ...
  int xRight = display.width() - updateStrWidth - 19;
  ...
  for (int i = 0; i < maxBoxes && i < count; i++) {
    ...
    display.drawTextBox(leftMargin, y0 + 5, display.width() - rightMargin,
                        y0 + 80, entities[i].title, 1, &GT_Pressura16pt7b, 30,
                        false, 18);

    display.drawTextBox(leftMargin, y0 + 80, display.width() - rightMargin,
                        y1 + 20, entities[i].description, 1, &Inter12pt7b, 30,
                        false, 16);
  }
}
