...
Fetch top news headlines from NewsAPI.org over WiFi and render a "World News" layout on Soldered Inkplate 10, refreshing periodically via deep sleep.

## Overview

Connects Inkplate 10 to WiFi (credentials configured via menuconfig), synchronizes time via NTP, and fetches the `top-headlines` endpoint from [NewsAPI.org](https://newsapi.org/) through the `NetworkFunctions` helper (`main/Network.cpp` + `main/Network.h`), which performs the HTTPS request with `esp_http_client` (verified against the ESP-IDF certificate bundle) and parses the JSON response with cJSON.

`main.cpp` then renders a simple newspaper-style screen: a "World News" title, the current date and last-update time, and a list of headline/description boxes drawn with `drawTextBox()` using three custom fonts (`FreeSerifItalic24pt7b` for the title, `Inter12pt7b` for the date row and descriptions, `GT_Pressura16pt7b` for the headlines). The display is rotated with `setRotation(3)` before drawing, matching the original Arduino sketch; all layout coordinates are computed from `display.width()`/`display.height()` after that rotation, so they adapt to the resulting canvas automatically.
...
- Inkplate 10's panel is 1200x825 (`E_INK_WIDTH`/`E_INK_HEIGHT`); after `setRotation(3)`, `display.width()`/`display.height()` report 825/1200. The layout constants in `main.cpp` (title position, divider lines, headline box sizing) are carried over directly from the original `Inkplate10_News.ino` sketch, which was already tuned for that exact rotated canvas — they are not rescaled from the smaller Inkplate 6 port.
