 ...
 * @brief       OpenAI text prompt example for Soldered Inkplate 13SPECTRA.
 *
 * @details     Connects Inkplate 13SPECTRA to WiFi, sends a text prompt to
 *              the OpenAI Chat Completions API over HTTPS, and renders the
 *              text response on the e-paper screen inside a word-wrapped
 *              text box using a custom font (FreeMonoBold18pt7b).
 *
 ...
 *
 * Expected output:
 * - Display: the OpenAI text response, word-wrapped inside a text box and
 *   rendered with the FreeMonoBold18pt7b font.
 ...
 * - Inkplate 13SPECTRA is a 6-color e-paper board (1200x1600): black, white,
 *   yellow, red, blue, green. Unlike Inkplate 6Color (7 colors, with an
 *   extra orange), there is no INKPLATE_ORANGE here - this example only
 *   ever needs INKPLATE_BLACK / INKPLATE_WHITE for text, so the missing
 *   color has no effect on this port.
 * - The reply is rendered with drawTextBox() at (50, 60)-(800, 900) with a
 *   58px line spacing and a 32pt font size. Those numbers are taken directly
 *   from the original Inkplate13SPECTRA_OpenAI_Text_Prompt.ino sketch (tuned
 *   for this board's 1200x1600 canvas), not rescaled from the Inkplate
 *   6Color port, whose box - (50, 50)-(550, 433), 38px spacing, 28pt - is
 *   sized for that board's much smaller 600x448 canvas.
...
extern "C" void app_main(void) {
  ...
  if (ok) {
    ...
    // Coordinates, line spacing (58px) and font size (32pt) match the
    // original Inkplate13SPECTRA_OpenAI_Text_Prompt.ino sketch, tuned for
    // this board's 1200x1600 canvas.
    display.drawTextBox(50, 60, 800, 900, reply, 1, &FreeMonoBold18pt7b, 58,
                        false, 32);
  } else {
