 ...
 * original stored Arduino String fields inside a fixed-size Event array;
 * this port keeps the same shape but uses fixed char[] buffers instead of
 * String to avoid heap fragmentation on the ESP32.
 */
#pragma once
...
// for all-day events). EVENT_SUMMARY_LEN comfortably fits the original
// sketch's MAX_SUMMARY_LENGTH (70, see Gui.h) plus the "..." suffix.
constexpr size_t EVENT_SUMMARY_LEN = 128;
