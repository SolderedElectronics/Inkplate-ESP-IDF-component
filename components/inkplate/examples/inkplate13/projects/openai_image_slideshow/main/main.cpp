 ...
 * @brief       OpenAI image generation slideshow for Soldered Inkplate 13SPECTRA.
 *
 * @details     Connects Inkplate 13SPECTRA to WiFi, sends a text prompt to
 *              OpenAI's image generation API (DALL-E), parses the JSON
 *              response to extract the generated image's URL, downloads that
 *              image, and renders it on the 6-color e-paper display. The
 *              board then schedules its next wake-up using the on-board RTC
 *              alarm and enters deep sleep, so the whole flow repeats
 *              periodically from app_main() on every wake-up.
 *
 *              Status messages ("Connecting...", "Generating image...", etc.)
 *              and the final downloaded image are all shown with full
 *              refreshes (display.display()), matching the behavior of the
 *              original Arduino sketch for this board.
 *
 ...
 * 4) Build and flash to Inkplate 13SPECTRA, then open the Serial Monitor.
 ...
 *
 * Expected output:
 * - During startup: short status messages on the display (full refresh
 *   each time).
 * - After generation: the downloaded image rendered on the e-paper display,
 *   dithered down to the panel's 6-color palette.
 ...
 *   downloaded image down to the panel's 6 colors (black, white, yellow,
 *   red, blue, green — there is no orange on this panel, unlike Inkplate
 *   6Color's 7-color palette). ImageColor already knows about this palette
 *   internally (see components/inkplate/src/graphics/ImageColor.cpp, which
 *   branches on CONFIG_INKPLATE_BOARD_INKPLATE13 to load the correct 6-entry
 *   RGB palette), so no palette handling is needed here. The draw()
 *   signature is the same shape as on grayscale/other color boards:
 *   draw(url, x, y, dither, invert).
 * - Image placement: the original Inkplate13SPECTRA Arduino sketch draws the
 *   downloaded (1024x1024) image at (0, 0) — i.e. top-left aligned, not
 *   centered on the 1200x1600 panel — despite its code comment saying
 *   "centered". This is carried over as-is rather than "fixed", to match
 *   real device behavior. (This differs from the Inkplate 6Color port, which
 *   centers the image using a 512 px half-width/half-height offset.)
 * - Request body: the original Inkplate13SPECTRA sketch's OpenAI request
 *   only sets "prompt", "n" and "size" (no "model"/"style" fields), which
 *   means it targets OpenAI's default image model rather than explicitly
 *   requesting dall-e-3 with vivid style like the Inkplate 6Color sketch
 *   does. This example matches that and omits OPENAI_IMAGE_MODEL/STYLE.
 * - Display mode: every status update and the final image use a full
 *   display.display() refresh, matching the original Arduino sketch's
 *   behavior for this board.
 ...
 *   ImageColor::PNG argument in the original sketch).
 * - RAM and bandwidth: downloading/decoding large images can be slow and
 ...
 *   an external wake on GPIO 18 (tied to the RTC interrupt line on Inkplate
 *   13SPECTRA), matching the original sketch and the other Inkplate 13
 *   deep-sleep/RTC examples in this component.
...
#define IMAGE_PROMPT                                                          \
  "Generate an image at 1024x1024 resolution with a lot of flowers "          \
  "including blue, red, yellow, orange and green colors. They should be on " \
  "a green hill with a clear blue sky in the background."
...
static bool requestOpenAIImage(const char *prompt, char *outUrl,
  ...
  // (No "model"/"style" fields, matching the original Inkplate13SPECTRA
  // sketch, which targets OpenAI's default image model.)
...
extern "C" void app_main(void) {
  ...
  if (gotUrl) {
    ...
    // Draw the image top-left aligned (matches the original
    // Inkplate13SPECTRA sketch, which draws at (0, 0) rather than centering
    // it on the panel).
    bool drawn = display.image.draw(imageUrl, 0, 0, true, false);
  ...
  // Enable external wakeup on GPIO 18 (tied to the RTC alarm interrupt on
  // Inkplate 13SPECTRA).
