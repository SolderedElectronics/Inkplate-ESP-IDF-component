 ...
 *              Arduino example's src/NetworkFunctions.cpp, which is
 *              identical (byte-for-byte, modulo the class living in a file
 *              named NetworkFunctions.cpp instead of Network.cpp) to the
 *              Inkplate6COLOR sibling's src/Network.cpp. See Network.h for
 *              a summary of what changed compared to the original
 *              HTTPClient + ArduinoJson version. The Open-Meteo query
 *              parameters are unchanged from the original sketch, even
 *              though this board's Gui.cpp only displays a subset of them.
 *
...
bool NetworkFunctions::setupTime(int utcOffsetHours, const char *ntpServer)
{
    ...
    // "UTC-1" means local time = UTC - (-1) = UTC+1.
    char tz[16];
