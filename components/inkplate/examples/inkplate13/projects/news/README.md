...
Fetch top news headlines from NewsAPI.org over WiFi and render a "World News" layout on Soldered Inkplate 13SPECTRA, refreshing periodically via deep sleep.

## Overview

Connects Inkplate 13SPECTRA to WiFi (credentials configured via menuconfig), synchronizes time via NTP, and fetches the `top-headlines` endpoint from [NewsAPI.org](https://newsapi.org/) through the `NetworkFunctions` helper (`main/Network.cpp` + `main/Network.h`), which performs the HTTPS request with `esp_http_client` (verified against the ESP-IDF certificate bundle) and parses the JSON response with cJSON.

`main.cpp` then renders a simple newspaper-style screen: a "World News" title, the current date and last-update time, and a list of headline/description boxes drawn with `drawTextBox()` using two custom fonts (`FreeSerifItalic24pt7b` for the title, `FreeSerifBold12pt7b` for the headlines, `Inter10pt7b` for the date row and descriptions). All layout coordinates are computed from `display.width()`/`display.height()`, so they adapt to the board's rotation automatically.
...
- Display mode: Inkplate 13SPECTRA is a 6-color e-paper board (black, white, yellow, red, blue, green via the `INKPLATE_*` color macros — see `Inkplate13.h`). Unlike Inkplate 6Color, there is **no `INKPLATE_ORANGE`** on this board. There is no `setDisplayMode()` call — it always renders in its native color mode. This example only draws with `INKPLATE_BLACK`/`INKPLATE_WHITE`, matching the original sketch's layout, and does not support partial updates (every refresh is a full refresh via `display.display()`).
- Orientation: `Inkplate::Inkplate()` already calls `setRotation(3)` by default for `CONFIG_INKPLATE_BOARD_INKPLATE13`. The original `Inkplate13SPECTRA_News.ino` calls `inkplate.setRotation(3)` again inside `drawNews()`, but since that's the board's already-active rotation, this port omits the redundant call (unlike this component's Inkplate 6Color `news` port, whose board defaults to rotation 2, so an explicit `setRotation(3)` is actually needed there).
- Deep sleep resets the ESP32 on every wake — there is no `loop()`; all logic runs once per boot inside `app_main()`.
- `display.wifi.setCurrentTime()` sets the system TZ internally, but `time()` always returns UTC seconds regardless of TZ, so this example applies `TIMEZONE_OFFSET_HOURS` manually (same approach as this component's Inkplate 6 and Inkplate 6Color `news` examples, and equivalent to the original sketch's fixed `timeZone` variable).
