...
//
int AnimatedGIF::open(uint8_t *pData, int iDataSize,
...
#ifdef ARDUINO
int AnimatedGIF::openFLASH(uint8_t *pData, int iDataSize,
                           GIF_DRAW_CALLBACK *pfnDraw) {
  _gif.iError = GIF_SUCCESS;
...
#endif
void AnimatedGIF::mergeTransparent(uint8_t *pSrc, uint8_t *pDst,
                                   uint8_t ucTrans, int iLen) {
  GIF_mergeTransparent(pSrc, pDst, ucTrans, iLen);
} /* mergeTransparent() */

void AnimatedGIF::cookPixels(uint8_t *pSrc, uint8_t *pDst, int iTrans, int iLen,
...
int AnimatedGIF::allocFrameBuf(GIF_ALLOC_CALLBACK *pfnAlloc) {
  if (_gif.iCanvasWidth > 0 && _gif.iCanvasHeight > 0 &&
...
int AnimatedGIF::allocTurboBuf(GIF_ALLOC_CALLBACK *pfnAlloc) {
  if (_gif.iCanvasWidth > 0 && _gif.iCanvasHeight > 0 &&
    ...
    // as RGB565 or RGB888
    int iTurboSize =
        TURBO_BUFFER_SIZE + (_gif.iCanvasWidth * _gif.iCanvasHeight);
    if (pfnAlloc == nullptr) {
...
int AnimatedGIF::getFrameWidth() { return _gif.iWidth; }
int AnimatedGIF::getFrameHeight() { return _gif.iHeight; }

int AnimatedGIF::getFrameXOff() { return _gif.iX; } /* AnimatedGIF() */

int AnimatedGIF::getFrameYOff() { return _gif.iY; }
...
int AnimatedGIF::getLastError() { return _gif.iError; } /* getLastError() */
#ifdef __LINUX__
...
//
int AnimatedGIF::open(const char *szFilename, GIF_OPEN_CALLBACK *pfnOpen,
...
void AnimatedGIF::begin(unsigned char ucPaletteType) {
  ...
  if (ucPaletteType != GIF_PALETTE_RGB565_LE &&
      ucPaletteType != GIF_PALETTE_RGB565_BE &&
...
// 0 = no more frames exist, a frame may or may not have been played: use
// getLastError() and look for GIF_SUCCESS to know if a frame was played -1 =
// error
