...
SPI::SPI(gpio_num_t mosi, gpio_num_t clk, gpio_num_t cs, spi_host_device_t host)
    : m_mosi(mosi), m_clk(clk), m_cs(cs), m_host(host)
{}

bool SPI::init()
{
    if (m_spiDev) return true; // already up
