...
const uint8_t BME680_I2C_MAX_ADDRESS{0x77};           ///< Maximum possible address for BME680
...
bool BME680Driver::begin(i2c_master_bus_handle_t busHandle, const uint8_t i2cAddress)
{
    ...
    @details Probes I2C addresses 0x76 and 0x77 (or only the specified address when
             i2cAddress != 0). On a successful ACK the device handle is created and
    ...
    */
         ...
         _I2CAddress++)
    {
        ...
        if (i2c_master_bus_add_device(busHandle, &devConfig, &_i2cDevHandle) == ESP_OK)
        {
...
bool BME680Driver::commonInitialization()
{
    ...
    @details Called from begin() once the I2C device handle is ready.
             Verifies chip ID, loads calibration, and triggers the first measurement.
    @return  true on success
    */
    ...
    if (_I2CAddress == 0 && ((SPI_Register >> BME680_SPI_MEM_PAGE_POSITION) & 1))
    {
...
uint8_t BME680Driver::readSensors(const bool waitSwitch)
{
    ...
    adc_pres    = (uint32_t)(((uint32_t)buff[2] << 12) | ((uint32_t)buff[3] << 4) | ((uint32_t)buff[4] >> 4));
    adc_temp    = (uint32_t)(((uint32_t)buff[5] << 12) | ((uint32_t)buff[6] << 4) | ((uint32_t)buff[7] >> 4));
    ...
    var2 = ((int32_t)_H2 *
            (((temp_scaled * (int32_t)_H4) / ((int32_t)100)) +
             (((temp_scaled * ((temp_scaled * (int32_t)_H5) / ((int32_t)100))) >> 6) /
              ((int32_t)100)) +
             (int32_t)(1 << 14))) >>
           10;
    var3      = var1 * var2;
...
bool BME680Driver::setGas(uint16_t GasTemp, uint16_t GasMillis) const
{
    ...
    {
        putData(BME680_CONTROL_GAS_REGISTER1, (uint8_t)0x08); // turn off heater (0b00001000)
        putData(BME680_CONTROL_GAS_REGISTER2, (uint8_t)(gasRegister & 0xEF)); // clear run_gas (0b11101111)
    }
    else
    {
        ...
        if (GasTemp < 200) GasTemp = 200;
        else if (GasTemp > 400) GasTemp = 400;

        var1 = (((int32_t)(_Temperature / 100) * _H3) / 1000) << 8;
        var2 = (_G1 + 784) * (((((_G2 + 154009) * GasTemp * 5) / 100) + 3276800) / 10);
        var3 = var1 + (var2 / 2);
        ...
        putData(BME680_CONTROL_GAS_REGISTER2, (uint8_t)(gasRegister | 0x10)); // set run_gas (0b00010000)
    }
    return true;
} // of method setGas()
